...
import group.aelysium.particulaterenderer.lib.renderEngine.EffectRender;
import org.bukkit.*;
...
public abstract class Effect {
    ...
    public void run(Effect effect,String name) {
        ...
        if (!effectArguments.containsKey("type"))
            ParticulateRenderer.getInstance().log("All effects must have a `type` defined!");
        else
        switch (Effect.Type.valueOf((String) effectArguments.get("type"))) {
            case PARTICLE:
                if (effectArguments.containsKey("particle")) {
                    ...
                    ParticulateRenderer.getInstance().log("Effect.Type.PARTICLE requires 'particle' to be defined");
                }
                break;
            case BLOCK:
                if (effectArguments.containsKey("material")) {
                    ...
                    ParticulateRenderer.getInstance().log("Effect.Type.BLOCK requires 'material'");
                }
                break;
            case FIREWORK:
            ...
            case CRYSTAL:
                ...
                if (effectArguments.containsKey("targetX") || effectArguments.containsKey("targetY") || effectArguments.containsKey("targetZ"))
                    ...
                    ParticulateRenderer.getInstance().log("Effect.Type.CRYSTAL requires targetX, targetY, and targetZ");
                    break;
                }
    ...
    public void detonateFirework(final Firework firework){
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(ParticulateRenderer.getInstance(), new Runnable() {
            public void run() {
                try{
                    firework.detonate();
                }catch(Exception e){}
    ...
    public static final class Builder {
        ...
        public Builder unwrap(String effectID) throws Throwable {
            ...
            if(ParticulateRenderer.getInstance().getMap("effects").containsKey("effects."+effectID)) {
                for (Map.Entry<String, Object> effectLoopMap : ParticulateRenderer.getInstance().getMap("effects").entrySet()) {
